...
function CalendarioPHP($year, $month, $day_heading_length = 3,$idv,$idc){ 
...
$sql="select * from diasfestivos where año='".$year."'"; 
...
for ($l=0;$l<$row;$l++){;
$df=mysql_result($result,$l,'dia'); 
$mf=mysql_result($result,$l,'mes'); 
...
/*$sql1="select * from noticias where idvecino='".$idv."' and idcomunidad='".$idc."' and estado='1' and evecino='1' and fechacreacion='".$fecha_b."'"; 
$result1=mysql_query ($sql1) or die ("Invalid query 1");
$array1=mysql_fetch_array($result1);
$row1=mysql_affected_rows();
$col=mysql_num_fields($result1);

$sql11="select * from noticias where idcomunidad='".$idc."' and estado='1' and ecomunidad='1' and fechacreacion='".$fecha_b."'"; 
$result11=mysql_query ($sql11) or die ("Invalid query 11");
$array11=mysql_fetch_array($result11);
$row11=mysql_affected_rows();
$col1=mysql_num_fields($result11);
*/

$link="";

for ($i=0; $i<$row1; $i++){;
$dato=mysql_result($result1,$i,'notas');
$link=$link.$dato;
};

for ($j=0; $j<$row11; $j++){;
$dato11=mysql_result($result11,$j,'notas');
$link11=$link.$dato11;
};
...
if($row1!=0){;
$calendar .= "<a href='mensaje.php?idv=$idv&idc=$idc&fc=$fecha_b' title='".$link."'>";
};

if($row11!=0){;
$calendar .= "<a href='mensaje.php?idv=$idv&idc=$idc&fc=$fecha_b' title='".$link11."'>";
};
$calendar .= "<font color='"; 
...
echo CalendarioPHP($año, $mes, $dia,$idvecino,$idcomu); 
?> 
</body>
