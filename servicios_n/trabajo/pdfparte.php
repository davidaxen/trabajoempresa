...
$sql1="SELECT * from acctrabajos where id='".$id."'"; 
...
if ($firmaaseg!=''){;
$this->Image('../../img/'.$firmaaseg,400,70,180,150);
//$this->Image($imgemp,400,70,0,50);
};

	 $this->SetXY(400,60);
    $this->MultiCell(180,12,'Firma ',0,'L');
	 $this->SetXY(400,70);
...
$at=350;
    ...
    $this->SetXY($jk,60);
    $this->MultiCell($at,$gt,'Num. Trabajo '.strtoupper($numsiniestro),0,'L');
    $this->SetX($jk);
...
$jk=$t+50;   
    $this->SetFont('Arial','B',14);
    $this->SetX($jk);
    $this->Cell(0,20,'Datos facilitados del trabajo ',0,1,'L');
    ...
    $this->SetFont('Arial','B',14);
    $this->SetX($jk);
    $this->Cell(0,20,'Descripcion de lo realizado ',0,1,'L');
    $this->SetFont('Arial','',10);
    $this->SetX($jk);
    $this->MultiCell(0,20,strtoupper($descripcion1),0,'L');  
    

$sqldes="SELECT * from imgtrabajos where idempresa='".$ide."' and idsiniestro='".$idsiniestro."' and dia='".$diasal."'";
//	echo $sqldes;
$resultdes=mysqli_query ($conn,$sqldes) or die ("Invalid result");
$rowdes=mysqli_num_rows($resultes);
if ($rowdes!=0){;
...
$resultadodes=mysqli_fetch_array($resultdes);
				...
				$xf=$this->GetX();
				$yf=$this->GetY();
				$this->Image('../../img/'.$imgsiniestro,$xf,$yf,500,300);
...
$this->Ln();
		...
		for ($tdes=0;$tdes<$rowdes;$tdes++){;
				...
				$xf=$this->GetX();
				$yf=$this->GetY();
				$this->Image('../../img/'.$imgsiniestro,$xf,$yf,550,350);
				$this->Ln(355);  
				$yf=$this->GetY();
				if ($yf>620){;
				$this->AddPage();
